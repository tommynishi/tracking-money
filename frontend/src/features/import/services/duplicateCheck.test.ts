...
const existing: EntryDuplicateKey[] = [
  { usedOn: "2026-06-24", amount: 315, normalizedDescription: "ファミリーマート" },
];

describe("markDuplicateRows", () => {
  it("既存明細と一致する行に重複フラグを付ける（摘要は正規化して比較・FR-DUP-01）", async () => {
    ...
    expect(rows.map((row) => row.isDuplicate)).toEqual([true, false, false]);
