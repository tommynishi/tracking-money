...
| 1-8 | 明細CRUD（一覧・登録・編集・削除・SCR-03/04） | 1-7 |
| 1-9 | ダッシュボード（今月合計・内訳・直近明細・SCR-02） | 1-8 |
| 1-10 | アカウント設定（SCR-08） | 1-2 |

**完了条件**：LINEログイン〜家族招待〜明細手入力〜ダッシュボード表示までが本番環境で動作し、認可のIntegration Testが全リソースで通る。
...
| 3-6 | 分析画面（SCR-11）・ダッシュボード拡充（前年同月比・AI所見） | 3-5 |

**完了条件**：LINE通知が設定どおり届き、分析画面の全タブが動作する。AI障害時も集計・グラフが表示される。
