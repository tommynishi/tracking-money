...
| FAMILY_LEDGER_EXISTS | 招待承諾時、自分の家族家計簿が既に存在（409・FR-INVITE-03） |
...
| エラー | 条件 |
| --- | --- |
| 409 FAMILY_LEDGER_EXISTS | 自分の家族家計簿が存在し `deleteOwnFamilyLedger` が false。true を指定して再実行すると自帳簿を論理削除して参加する |
