...
| SCR-02 | ダッシュボード | `/dashboard` | 1※ | 今月の支出サマリー（※集計表示はPhase 3で拡充） |
| SCR-03 | 明細一覧 | `/entries` | 1 | 明細の一覧・検索・絞り込み |
...
```mermaid
graph LR
    LOGIN[SCR-01 ログイン] -->|LINE認証成功| DASH[SCR-02 ダッシュボード]
...
* 切替状態は全画面（明細・取込・分析・カテゴリ）へ共通に適用する
* 家族家計簿未参加の場合、切替メニューに「家族家計簿を作成」導線を表示する
...
| 項目 | 内容 |
| --- | --- |
| 表示 | 今月支出合計／前月比（増減額・%）／カテゴリ別内訳（円グラフ or 横棒）／直近明細（5件） |
| 表示（Phase 3） | 前年同月比／AI所見サマリー（SCR-11への導線） |
...
| 項目 | 内容 |
| --- | --- |
| 表示（PC） | 表形式：利用日／摘要／カテゴリ／金額／支払方法／登録者（家族帳簿のみ）／取込元アイコン |
| 表示（スマホ） | カード形式：日付見出しごとにグループ表示。カードに摘要・カテゴリ・金額 |
| 絞り込み | 月切替（前月・翌月ナビ）／カテゴリ／金額範囲／キーワード／取込元 |
| ソート | 利用日（既定：降順）／金額 |
...
使用API：`GET / PATCH / DELETE /api/ledgers/{id}`、`GET / DELETE …/members`、`POST …/invitations`、`GET /api/users/search`
...
| 項目 | 内容 |
| --- | --- |
| 表示 | ドラッグ&ドロップ＋ファイル選択（CSV / PDF・最大10MB）／フォーマット選択（既定：自動判定） |
...
### Step 2：プレビュー・確認（FR-CSV-04 / FR-PDF-02）

| 項目 | 内容 |
| --- | --- |
| 表示（PC） | 表形式：取込チェックボックス／利用日／摘要／金額／カテゴリ（編集可）／重複バッジ |
| 表示（スマホ） | カード形式で同内容 |
...
| 項目 | 内容 |
| --- | --- |
| 表示 | 取込日時／ファイル名／フォーマット／件数（取込・スキップ・エラー）／状態／Driveリンク（FR-DRIVE-05） |
| 操作 | ダウンロード／Driveファイル削除（確認ダイアログ。明細は残る旨を明示・FR-DRIVE-04）／詳細表示（エラー行の内容） |

## SCR-11 分析

使用API：`GET …/analysis/*`（summary / trend / ranking / subscriptions / insight）
