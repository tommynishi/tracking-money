...
「今月」は支払月ベース。毎月10日を締め日とし、10日を含めそれ以前は当月、10日を超えたら翌月として扱う（`currentBillingMonth`。api.md 9章）。
...
| 項目 | 内容 |
| --- | --- |
| 表示（PC） | 表形式：利用日／支払月／摘要／カテゴリ／金額／支払方法／登録者（家族帳簿のみ）／取込元アイコン |
...
| 項目 | 内容 |
| --- | --- |
| 表示（PC） | 表形式：取込チェックボックス／利用日／支払月（編集可・既定はStep1の値）／摘要／金額／カテゴリ（編集可）／備考（編集可）／重複バッジ |
| 表示（スマホ） | カード形式で同内容 |
